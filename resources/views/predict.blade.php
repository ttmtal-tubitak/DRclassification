    ...
    <body class="antialiased">
        <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-white-900 sm:items-center py-4 sm:pt-0" style="display: flex; justify-content: flex-start;">
            ...
            <div class="hidden fixed top-0 left-0 px-6 py-4 sm:block" style="margin-left: 15%;">
                <a href="{{ url('/') }}" class="text-sm text-gray-700 dark:text-gray-500 underline"  style="text-decoration: none; font-size: 1.3em; margin-right: 1em;">{{ __('auth.home') }}</a>
                <a href="{{ url('/predict') }}" class="text-sm text-gray-700 dark:text-gray-500 underline"  style="border-bottom: 1px solid #302def; padding-bottom: 7px;text-decoration: none; font-size: 1.3em; margin-right: 1em;">Teşhis</a>
            ...
            <div class="container" style="color: #2d3748; font-size:21px;">
                    ...
                    <div class="col-12">
                        <form method="POST" enctype="multipart/form-data" id="image-upload" action="javascript:void(0)" >
                            <div class="input-group">
                                <input type="file" name="image" class="form-control" id="image" aria-describedby="inputGroupFileAddon04" aria-label="Upload" required="re">
                                {{-- <button class="btn btn-light btn-outline-primary" type="button" id="inputGroupFileAddon04">Button</button> --}}
                            </div>
                            <div class="d-grid gap-2 mt-3">
                                <button type="submit" class="btn btn-block btn-outline-primary">İncele</button>
                            </div>
                            {{ csrf_field() }}
                        </form>
                    </div>
                ...
                <div class="row mt-3" id="res" style="min-height: 10em; display: none;">
                    <div class="col-lg-4 mb-2" id="imageBox">
                        <img src="/img/preds/date_13-01-2024_11-26-52-1891025547.jpg" class="img-thumbnail" style="min-width: 100%; min-height: 100%;" alt="">
                    </div>
                    <div class="col-lg-8">
        ...
        $('#image-upload').submit(function(e) {
            ...
            $.ajax({
                ...
                success: (datas) => {
                    ...
                    indices.reverse();
                    for(let i=0; i<=4 ; i++){
                        if(indices[i] == 0) indices[i] = "Sağlıklı "
                         else if(indices[i] == 1) indices[i] = "Hafif DR"
                         else if(indices[i] == 2) indices[i] = "Orta Dr"
                         else if(indices[i] == 3) indices[i] = "Şiddetli DR"
                         else if(indices[i] == 4) indices[i] = "Proliferatif DR"
                    /*
                        if(indices[i] == 0) indices[i] = "Infectious Skin Disease"
                        else if(indices[i] == 1) indices[i] = "Eczama"
                        else if(indices[i] == 2) indices[i] = "Acne"
                        else if(indices[i] == 3) indices[i] = "Pigment-Induced Skin Disease"
                        else if(indices[i] == 4) indices[i] = "Benign Skin Tumor"
                        else if(indices[i] == 5) indices[i] = "Malign Skin Tumor"
                    */
                    }
