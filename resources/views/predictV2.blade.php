        ...
        <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0" style="display: flex; justify-content: flex-start;">
            ...
            <div class="container" style="color: #e2e8f0; font-size:21px;">
                <div class="flex justify-center pt-8 sm:justify-start sm:pt-0" style="font-family: 'Nunito', sans-serif; font-size: 1.2em; color: #ef3c2d; font-weight: 900; justify-content: space-between;">
                    {{ __('main.title') }} - Geliştirilmiş Model (V2) 
                ...
                <div class="row mt-3" id="res" style="min-height: 10em; display: none;">
                    ...
                    <div class="col-lg-8">
                        <div class="list-group">
                            <a href="#" class="list-group-item list-group-item-action" id="oneLink" aria-current="true">
                              <div class="d-flex w-100 justify-content-between">
                                <h5 style="font-size: 1.2em" class="mb-2" id="oneName"></h5>
                                <small style="font-size: 1.2em" id="onePred"></small>
                              </div>
                            </a>
                            <a href="#" class="list-group-item list-group-item-action" id="twoLink" aria-current="true">
                                <div class="d-flex w-100 justify-content-between">
                                  <h5 style="font-size: 1.2em" class="mb-1" id="twoName"></h5>
                                  <small style="font-size: 1.2em" id="twoPred"></small>
                                </div>
                            </a>
                            <a href="#" class="list-group-item list-group-item-action" id="threeLink" aria-current="true">
                                <div class="d-flex w-100 justify-content-between">
                                  <h5 style="font-size: 1.2em" class="mb-1" id="threeName"></h5>
                                  <small style="font-size: 1.2em" id="threePred"></small>
                                </div>
                            </a>
                            <a href="#" class="list-group-item list-group-item-action" id="fourLink" aria-current="true">
                                <div class="d-flex w-100 justify-content-between">
...
<script>
    ...
    let colorOne = '#005c00';
    let colorTwo = '#2d661b';
    let colorThree = '#2a850e';
    let colorFour = '#27a300';
    let colorFive = '#2cbe00';
    let colorSix = '#9fd859';
    let colorSeven = '#cbff8c';
        ...
        $('#image-upload').submit(function(e) {
            ...
            $.ajax({
                ...
                success: (datas) => {
                    ...
                    $('#onePred').html(`% ${parseInt(Number(result[0]).toFixed(2) * 100)}`)

                    $('#twoName').html(`${indices[1]}`)
                    $('#twoPred').html(`% ${parseInt(Number(result[1]).toFixed(2) * 100)}`)

                    $('#threeName').html(`${indices[2]}`)
                    $('#threePred').html(`% ${parseInt(Number(result[2]).toFixed(2) * 100)}`)

                    $('#fourName').html(`${indices[3]}`)
                    $('#fourPred').html(`% ${parseInt(Number(result[3]).toFixed(2) * 100)}`)
                    
                    $('#fiveName').html(`${indices[4]}`)
                    $('#fivePred').html(`% ${ parseInt(Number(result[4]).toFixed(2) * 100) }`)

                    $('#sixName').html(`${indices[5]}`)
                    $('#sixPred').html(`% ${parseInt(Number(result[5]).toFixed(2) * 100)}`)
                    ...
                    /*
                        $('#oneLink').css('background-color', `rgba(0, 92, 0, ${Number(result[0]).toFixed(2)})`)
                        $('#twoLink').css('background-color', `rgba(0, 92, 0, ${Number(result[1]).toFixed(2)})`)
                        $('#threeLink').css('background-color', `rgba(0, 92, 0, ${Number(result[2]).toFixed(2)})`)
                        $('#fourLink').css('background-color', `rgba(0, 92, 0, ${Number(result[3]).toFixed(2)})`)
                        $('#fiveLink').css('background-color', `rgba(0, 92, 0, ${Number(result[4]).toFixed(2)})`)
                        $('#sixLink').css('background-color', `rgba(0, 92, 0, ${Number(result[5]).toFixed(2)})`)
                    */
                   
                    /*
                        if(Number(result[0]).toFixed(2) < 0.15){
                            $('#oneLink').css('background-color', colorSeven)
                        }
                        else if(Number(result[0]).toFixed(2) > 0.15 && Number(result[0]).toFixed(2) < 0.30){
                            $('#oneLink').css('background-color', colorSix)
                        }
                        else if(Number(result[0]).toFixed(2) > 0.30 && Number(result[0]).toFixed(2) < 0.50){
                            $('#oneLink').css('background-color', colorFive)
                        }
                        else if(Number(result[0]).toFixed(2) > 0.50 && Number(result[0]).toFixed(2) < 0.65){
                            $('#oneLink').css('background-color', colorFour)
                        }
                        else if(Number(result[0]).toFixed(2) > 0.65 && Number(result[0]).toFixed(2) < 0.80){
                            $('#oneLink').css('background-color', colorThree)
                        }
                        else if(Number(result[0]).toFixed(2) > 0.80 && Number(result[0]).toFixed(2) < 0.90){
                            $('#oneLink').css('background-color', colorTwo)
                        }
                        else if(Number(result[0]).toFixed(2) > 0.90 && Number(result[0]).toFixed(2) <= 1){
                            $('#oneLink').css('background-color', colorOne)
                        }

                        if(Number(result[1]).toFixed(2) < 0.15){
                            $('#twoLink').css('background-color', colorSeven)
                        }
                        else if(Number(result[1]).toFixed(2) > 0.15 && Number(result[0]).toFixed(2) < 0.30){
                            $('#twoLink').css('background-color', colorSix)
                        }
                        else if(Number(result[1]).toFixed(2) > 0.30 && Number(result[0]).toFixed(2) < 0.50){
                            $('#twoLink').css('background-color', colorFive)
                        }
                        else if(Number(result[1]).toFixed(2) > 0.50 && Number(result[0]).toFixed(2) < 0.65){
                            $('#twoLink').css('background-color', colorFour)
                        }
                        else if(Number(result[1]).toFixed(2) > 0.65 && Number(result[0]).toFixed(2) < 0.80){
                            $('#twoLink').css('background-color', colorThree)
                        }
                        else if(Number(result[1]).toFixed(2) > 0.80 && Number(result[0]).toFixed(2) < 0.90){
                            $('#twoLink').css('background-color', colorTwo)
                        }
                        else if(Number(result[1]).toFixed(2) > 0.90 && Number(result[0]).toFixed(2) <= 1){
                            $('#twoLink').css('background-color', colorOne)
                        }

                        if(Number(result[2]).toFixed(2) < 0.15){
                            $('#threeLink').css('background-color', colorSeven)
                        }
                        else if(Number(result[2]).toFixed(2) > 0.15 && Number(result[0]).toFixed(2) < 0.30){
                            $('#threeLink').css('background-color', colorSix)
                        }
                        else if(Number(result[2]).toFixed(2) > 0.30 && Number(result[0]).toFixed(2) < 0.50){
                            $('#threeLink').css('background-color', colorFive)
                        }
                        else if(Number(result[2]).toFixed(2) > 0.50 && Number(result[0]).toFixed(2) < 0.65){
                            $('#threeLink').css('background-color', colorFour)
                        }
                        else if(Number(result[2]).toFixed(2) > 0.65 && Number(result[0]).toFixed(2) < 0.80){
                            $('#threeLink').css('background-color', colorThree)
                        }
                        else if(Number(result[2]).toFixed(2) > 0.80 && Number(result[0]).toFixed(2) < 0.90){
                            $('#threeLink').css('background-color', colorTwo)
                        }
                        else if(Number(result[2]).toFixed(2) > 0.90 && Number(result[0]).toFixed(2) <= 1){
                            $('#threeLink').css('background-color', colorOne)
                        }

                        if(Number(result[3]).toFixed(2) < 0.15){
                            $('#fourLink').css('background-color', colorSeven)
                        }
                        else if(Number(result[3]).toFixed(2) > 0.15 && Number(result[0]).toFixed(2) < 0.30){
                            $('#fourLink').css('background-color', colorSix)
                        }
                        else if(Number(result[3]).toFixed(2) > 0.30 && Number(result[0]).toFixed(2) < 0.50){
                            $('#fourLink').css('background-color', colorFive)
                        }
                        else if(Number(result[3]).toFixed(2) > 0.50 && Number(result[0]).toFixed(2) < 0.65){
                            $('#fourLink').css('background-color', colorFour)
                        }
                        else if(Number(result[3]).toFixed(2) > 0.65 && Number(result[0]).toFixed(2) < 0.80){
                            $('#fourLink').css('background-color', colorThree)
                        }
                        else if(Number(result[3]).toFixed(2) > 0.80 && Number(result[0]).toFixed(2) < 0.90){
                            $('#fourLink').css('background-color', colorTwo)
                        }
                        else if(Number(result[3]).toFixed(2) > 0.90 && Number(result[0]).toFixed(2) <= 1){
                            $('#fourLink').css('background-color', colorOne)
                        }

                        if(Number(result[4]).toFixed(2) < 0.15){
                            $('#fiveLink').css('background-color', colorSeven)
                        }
                        else if(Number(result[4]).toFixed(2) > 0.15 && Number(result[0]).toFixed(2) < 0.30){
                            $('#fiveLink').css('background-color', colorSix)
                        }
                        else if(Number(result[4]).toFixed(2) > 0.30 && Number(result[0]).toFixed(2) < 0.50){
                            $('#fiveLink').css('background-color', colorFive)
                        }
                        else if(Number(result[4]).toFixed(2) > 0.50 && Number(result[0]).toFixed(2) < 0.65){
                            $('#fiveLink').css('background-color', colorFour)
                        }
                        else if(Number(result[4]).toFixed(2) > 0.65 && Number(result[0]).toFixed(2) < 0.80){
                            $('#fiveLink').css('background-color', colorThree)
                        }
                        else if(Number(result[4]).toFixed(2) > 0.80 && Number(result[0]).toFixed(2) < 0.90){
                            $('#fiveLink').css('background-color', colorTwo)
                        }
                        else if(Number(result[4]).toFixed(2) > 0.90 && Number(result[0]).toFixed(2) <= 1){
                            $('#fiveLink').css('background-color', colorOne)
                        }
                    */
